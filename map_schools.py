...
/***************************************************************************
 MapSchool
                                 A QGIS plugin
 Map school, create buffer and report vulnerability
 Generated by Plugin Builder: http://g-sherman.github.io/Qgis-Plugin-Builder/
                              -------------------
        begin                : 2021-04-23
        git sha              : $Format:%H$
        copyright            : (C) 2021 by Infosys
        email                : hilalmohd180@gmail.com
 ***************************************************************************/
...
class MapSchool:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Map School')
    ...
    def tr(self, message):
        ...
        return QCoreApplication.translate('MapSchool', message)
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/map_schools/icon.png'
        self.add_action(
            icon_path,
            text=self.tr(u'Map Schools'),
            callback=self.run,
    ...
    def run(self):
        ...
        if self.first_start == True:
            ...
            self.dlg = MapSchoolDialog()
            self.dlg.cmbType.currentTextChanged.connect(self.evt_cmbType_changed)
            #QMessageBox.information(self.dlg,"Message","This is the first Message")
        ...
        #QMessageBox.information(self.dlg, "Message", "This should run every time")
        map_layers=[]
        for lyr in mc.layers():
            map_layers.append(lyr.name())
        QMessageBox.information(self.dlg,"Message","Layers are\n"+str(map_layers))
        ...
        if result:
            ...
            valLog = self.dlg.spbLongitude.value()
            valType = self.dlg.cmbType.currentText()
            valBuffer = self.dlg.spbBuffer.value()
            valFunctional = self.dlg.cmbFunctional.currentText()
            valDate = self.dlg.dteLast.date()
            QMessageBox.information(self.dlg,"Message","New Nest ID: {}\n\n Latitude: {}\n Longitude: {}\n Type: {}\n Buffer Distance: {}\n Functional: {}\n Date: {}".format(valNestId,valLat,valLog,valType,valBuffer,valFunctional,valDate))
            ftrNest=QgsFeature(lyrNest.fields())
            ftrNest.setAttribute("lat_y_dd",valLat)
            ftrNest.setAttribute("long_x_dd", valLog)
            ftrNest.setAttribute("recentspec", valType)
            ftrNest.setAttribute("recentstat", valFunctional)
            ftrNest.setAttribute("buf_dist", valBuffer)
            ftrNest.setAttribute("Nest_ID", valNestId)
            ftrNest.setAttribute("Lastsurvey", valDate)
            geom=QgsGeometry(QgsPoint(valLog,valLat))
            ftrNest.setGeometry(geom)
            prNest=lyrNest.dataProvider()
            prNest.addFeature(ftrNest)
            lyrNest.reload()

            prBuffer=lyrBuffer.dataProvider()
            ...
            pass
            #QMessageBox.information(self.dlg, "Message", "This should run only Cancel button clicked")

    def evt_cmbType_changed(self, type):
        if type=="High School":
            self.dlg.spbBuffer.setValue(0.004)
        elif type=="Higher Secondary School":
            self.dlg.spbBuffer.setValue(0.006)
        elif type=="Collage":
            self.dlg.spbBuffer.setValue(0.008)
        else:
            self.dlg.spbBuffer.setValue(0.00)
